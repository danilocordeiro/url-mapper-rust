...
macro_rules! sender_failed_json {
    ($m: expr, $f: tt) => {
        match $m {
            Ok(_) => {},
            Err(e) => {
                tracing::error!("Dtabase Manager failed to get {}! Error: {}", $f, e);
...
macro_rules! recv_failed_json {
    ($m: expr, $status: expr) => {
        match $m {
            Ok(d) => d,
            Err(e) => {
                tracing::error!("Dtabase Manager returned error: {}", e);
...
pub async fn get_url_map(req: Request<Body>) -> Result<Response<Body>> {
    ...
    let url_map = recv_failed_json!(rx.await.unwrap(), hyper::StatusCode::INTERNAL_SERVER_ERROR);
